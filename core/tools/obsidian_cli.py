...
- 本模块只封装 Ingestion / Vault 管理需要的几个核心操作：
  create / property_set / read / search
- 不暴露改 status 的便捷接口——status 修改只能人工在 Obsidian 完成（信任闸门）
- 所有命令异步执行，避免阻塞事件循环

⚠️ 信任闸门：本模块**不提供**任何能修改 status 字段的封装。
property_set 显式拒绝设置 status 字段，防止误用绕过人工 Promotion。
"""
...
async def create_note(
    ...
    if avail.kind == "yakitrak":
        ...
        # 无 Obsidian 桌面端时此路径会失败，调用方 fallback 文件系统（见 vault_io.create_staged_note）。
