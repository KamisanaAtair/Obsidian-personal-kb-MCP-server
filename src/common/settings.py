...
[公共文件] 被以下链调用（各链就近取用自己需要的字段）：
  - ingest_prepare    vault_path / inbox_path / video_to_text_mcp_enabled / whisper_*
  - ingest_finalize   vault_path / inbox_path / note_status_field / obsidian_skill_enabled
  - correlate         经 src/common/tools/retriever.py 取 rag_* / hybrid_* / chroma_persist_dir
  - query             同上
  另被 mcp_server/server.py 与 scripts/ 用于启动期校验。
"""
...
class Settings(BaseSettings):
    ...
    # ------------------------------------------------------------------
    # 【已注释 —— 对 LLM 默认不存在】
    # correlation_min_score 经全仓引用扫查确认无任何读取点，是死配置：关联链实际只用
    # correlation_top_k 截断候选，没有分数阈值过滤。保留注释仅供人工追溯历史，
    # 不代表当前设计契约：对本仓库的 LLM 而言，这个配置项默认不存在，
    # 不应被读取，也不应据此推断候选召回存在最低分门槛。
    # ------------------------------------------------------------------
    ...
    # 方法论在 src/ingest_prepare/tools/video_to_text.py 独立实现，产出纯文本（非 PDF）。
    ...
    # 本系统不依赖 Host 的 obsidian skill；在 src/ingest_finalize/tools/obsidian_cli.py 独立封装 CLI 适配层。
    ...
    def vault_path(self) -> Path:
        ...
        if self.vault_autodiscover:
            from src.common.tools.vault_resolver import resolve_vault_path
