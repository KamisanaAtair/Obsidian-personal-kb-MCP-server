"""链3 关联发现的节点：只检索候选，关联理由由 Host 生成。

[调用链] correlate —— 唯一入口 MCP 工具 trigger_correlation
  上游: src/correlate/graph.py
  下游: src/vault_io.py（读目标笔记 + 信任闸门）
        src/common/tools/retriever.py（ensure_index_fresh + retrieve）
        src/common/prompts.py（CORRELATION_SYSTEM / CORRELATION_USER）
...
from src.common.prompts import CORRELATION_SYSTEM, CORRELATION_USER
