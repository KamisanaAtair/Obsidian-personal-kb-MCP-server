...
[调用链] ingest_prepare —— 仅被 MCP 工具 ingest_content_prepare 一条链调用
  上游: src/ingestion.py 的 ingestion_prepare_node（transcribe_video / extract_video_*）
  副作用: 临时音频文件、whisper 模型缓存、网络下载（不在 Vault 内产生任何写入）
...
# ---------------------------------------------------------------------------
# 【已注释 —— 对 LLM 默认不存在】
# is_video_url / is_video_file 经全仓引用扫查确认无任何调用方：主链的来源判别实际走
# ingest_prepare 节点内的 _detect_source_type，它直接调用 extract_video_url / extract_video_file。
# 保留注释仅供人工追溯历史，**不代表当前设计契约**：对本仓库的 LLM 而言，这段代码
# 默认不存在，不应被引用。需要"是否包含视频引用"的布尔判断时用 extract_* is not None。
# ---------------------------------------------------------------------------
