...
[公共文件] 本文件被两条链共同使用，故整体放在 src 根目录，不做拆分：
  - ingest_prepare    ingestion_prepare_node      ← src/ingest_prepare/graph.py
  - ingest_finalize   ingestion_finalize_node     ← src/ingest_finalize/graph.py
  上游/下游:
    src/common/tools/session_store.py            会话配对（两条链之间唯一的耦合）
    src/ingest_prepare/tools/video_to_text.py    视频转写
    src/ingest_finalize/tools/obsidian_skill.py  frontmatter 校验 / 强制 staged
    src/vault_io.py                              create_staged_note 落盘
    src/common/prompts.py                        INGESTION_SYSTEM / INGESTION_USER

两个节点虽分属两条链，但同源同文件：拆分文件会割裂 _detect_source_type / _strip_ref
这类共用辅助，收益低于维护成本。
...
from src.common.prompts import INGESTION_SYSTEM, INGESTION_USER
from src.common.settings import get_settings
from src.common.state import KBState
from src.common.tools import session_store
from src.ingest_finalize.tools.obsidian_skill import _ensure_frontmatter, _force_status_staged
from src.ingest_prepare.tools.video_to_text import extract_video_file, extract_video_url, transcribe_video
...
async def ingestion_prepare_node(state: KBState) -> dict:
    ...
    if source_type == "video_url":
        ...
        from src.vault_io import read_note
...
async def ingestion_finalize_node(state: KBState) -> dict:
    ...
    from src.vault_io import create_staged_note
