...
[公共文件] 读侧多链共用、写侧只服务 ingest_finalize，故整体放在 src 根目录，不做拆分：
  - ingest_prepare   read_note（note_path 来源时读取原文）
  - ingest_finalize  create_staged_note（把 Host 正文强制写成 staged 草稿）
  - correlate        read_note（读目标笔记并判定 promoted）
  - query            间接：经 src/common/tools/retriever.py 调 list_promoted_notes 建索引
  另被 scripts/debug_run.py status 调 list_staged_notes。
...
async def create_staged_note(
    ...
    if s.obsidian_skill_enabled:
        try:
            from src.ingest_finalize.tools.obsidian_cli import create_note as cli_create_note, detect_cli
